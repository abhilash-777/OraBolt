...
const editCoupon = async (req,res) => {
  try {
    ...
    if(!name||!offerPrice||!minimumPrice||!endDate){
      return res.status(404).json({success:false,message:"All fields are required. Please fill the fields"});
    }
    const coupon = await Coupon.findById(couponId);
    const currentCouponStartDate = new Date(coupon.createdAt).toISOString().split('T')[0];
    if(!coupon)return res.status(404).json({success:false,message:"Coupon not found"});
    if(startDate && currentCouponStartDate !== startDate)return res.json({success:false,message:"Coupon start date cannot changeable"});
